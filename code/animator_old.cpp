...
void PlayingAnimation::initialize(Animation *animation, f32 weight, s32 parent, u32 flags) {
    ...
    memcpy(this->name, animation->name, strlen(animation->name));

}

void PlayingAnimation::flag_add(u32 flag) {
    this->flags |= flag;
}

void PlayingAnimation::flag_remove(u32 flag) {
    this->flags &= (~flag);
}

bool PlayingAnimation::flag_is_set(u32 flag) {
...
void Animator::update(f32 dt) {
    ...
    for(u32 animation_index = 0; animation_index < animation_queue_size; ++animation_index) {
        ...
        f32 animation_transition_time = MIN(playing->animation->duration/3, 1);
        ASSERT(animation_transition_time > 0);

        f32 t = 1;
        f32 t_left = playing->animation->duration - playing->time;
        
        if(playing->first_time_in_loop == true) {
            if(playing->time <= animation_transition_time) {
                f32 x = playing->time/animation_transition_time;
                t = CLAMP(1-(1-x)*(1-x), 0, 1);
            } else {
                playing->first_time_in_loop = false;
            }
        }

        
        if(t_left <= animation_transition_time) {
            f32 x = (animation_transition_time - t_left)/animation_transition_time;
            t = CLAMP(1-x*x, 0, 1);
        }

        mix_keyframes(&current_keyframe, prev, next, progression, playing->parent, &skeleton);
        mix_keyframes(&final_keyframe, final_keyframe, current_keyframe, playing->weight*t, playing->parent, &skeleton);
    }
...
void Animator::start_animation(const char *name, b32 loop, b32 smooth, f32 weight, const char *parent_bone_name) {

    if(animation_queue_size >= MAX_ANIMATION_QUEUE) return;
    
    if(animation_is_playing(name)) {
        stop_animation(name, smooth);
    }
    
    u32 flags = 0;

    if(smooth) {
        flags |= ANIMATION_SMOOTH;
        flags |= ANIMATION_SMOOTH_START;
    }
    if(loop) {
        flags |= ANIMATION_TO_LOOP;
    }

    s32 parent = find_bone_id_by_name(&skeleton, (char *)parent_bone_name);
    Animation *animation = find_animation_by_name(&skeleton, (char *)name);
    ASSERT(animation != nullptr);
    animation_queue[animation_queue_size++].initialize(animation, weight, parent, flags);
}


void Animator::stop_animation(const char *name, b32 smooth) {
    s32 animation_index = find_playing_animation_index(name);
    PlayingAnimation *animation = animation_queue + animation_index;
    if(smooth) {
        animation->flag_add(ANIMATION_SMOOTH_END);
    } else {
        animation->flag_add(ANIMATION_TO_REMOVE);
    }
}

bool Animator::animation_is_playing(const char *name) {
    return find_playing_animation_index(name) != -1;
}

void Animator::update_animations(f32 dt) {
    ...
    for(u32 animation_index = 0; animation_index < animation_queue_size; ++animation_index) {
        ...
        if(animation->time >= animation->animation->duration) {
            if(animation->flag_is_set(ANIMATION_TO_LOOP)) {
                animation->time = 0;
            } else {
                animation->flag_add(ANIMATION_TO_REMOVE);
            }
        ...
        if(animation->flag_is_set(ANIMATION_TO_REMOVE)) {
            remove_animation_from_queue(animation_index);
            --animation_index;
            continue;
        }

        if(animation->flag_is_set(ANIMATION_SMOOTH)) {

        }

        if(animation->flag_is_set(ANIMATION_SMOOTH_START)) {

        }

        if(animation->flag_is_set(ANIMATION_SMOOTH_END)) {
...
void Animator::remove_animation_from_queue(u32 animation_index) {
    u32 next_animation_index = animation_index + 1;
    u32 total_size_to_move = (animation_queue_size - next_animation_index)*sizeof(PlayingAnimation);
    memmove(animation_queue + animation_index, animation_queue + next_animation_index, total_size_to_move);
    --animation_queue_size;
...
s32 Animator::find_playing_animation_index(const char *name) {
    for(u32 animation_index = 0; animation_index < animation_queue_size; ++animation_index) {
        PlayingAnimation *playing = animation_queue + animation_index;
        if(strcmp(playing->name, name) == 0) {
